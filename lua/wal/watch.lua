--- @brief [[
--- File watcher module for the wal.nvim colorscheme
---
...
function FileWatcher.new(path, callback, opts)
	...
	if not self.event_handle then
		vim.notify("Error: wal.nvim unable to generate file watcher handle.", log.levels.ERROR)
	end
	return self
end
...
function FileWatcher:stop()
	if not uv.fs_event_stop(self.event_handle) then
		vim.notify("Error: wal.nvim unable to stop file watcher.", log.levels.ERROR)
		return false
	else
		return true
	end
...
function FileWatcher:start()
	local event_callback = function(err, filename, events)
		if err then
			self:stop()
			vim.notify("Error: file watcher" .. err, log.levels.ERROR)
		else
			self:stop()
	...
	if not uv.fs_event_start(self.event_handle, self.path, self.flags, event_callback) then
		vim.notify("Error: wal.nvim unable to start file watcher.", log.levels.ERROR)
		return false
	else
		return true
	end
